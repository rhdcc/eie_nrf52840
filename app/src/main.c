...
static void ble_on_device_disconnected(struct bt_conn* conn, uint8_t reason) {
    if(conn != my_connection) return;
    char *s = bt_hci_err_to_str(reason);
    printk("Disconnect reason: %s\n", s);
    bt_conn_unref(conn);
    my_connection = NULL;
}
...
static void ble_on_advertisement_received(const bt_addr_le_t* addr, int8_t rssi, uint8_t type,
    ...
    if(my_connection || !(type == BT_GAP_ADV_TYPE_ADV_IND || type == BT_GAP_ADV_TYPE_ADV_DIRECT_IND)) return;

    char name[32] = {0};
    bt_data_parse(ad, ble_get_adv_device_name_cb, name);
    char mac_str[10];
    bt_addr_le_to_str(addr, mac_str, 10);
    printk("MAC address: %s, Device name: %s, rssi: %d\n", mac_str, name, rssi);
    if(rssi < -50) return;

    bt_le_scan_stop();
    if(bt_conn_le_create(addr, BT_CONN_LE_CREATE_CONN, BT_LE_CONN_PARAM_DEFAULT, &my_connection)) {
